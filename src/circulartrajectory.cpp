...
void CircularTrajectory::computeDesired(const double actualTime)
{
    ...
    m_desiredPosition(0)= 0; //m_radius*(cos(m_angularSpeed*time) - 1); //so that the trajectory go through 0
    m_desiredPosition(1)= 200*time; //m_radius*sin(m_angularSpeed*time);
    //compute derivative
    m_desiredPositionDot(0)= 0; //-m_angularSpeed*m_radius*sin(m_angularSpeed*time);
    m_desiredPositionDot(1)= 200; //m_angularSpeed*m_radius*cos(m_angularSpeed*time);
}
